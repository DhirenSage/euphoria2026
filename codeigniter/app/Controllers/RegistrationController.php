...
class RegistrationController extends BaseController
{
    public function create(?string $slug = null)
    {
        ...
        if ($slug && (!$event || $event['status'] !== 'registration_open')) return redirect()->to('/events/'.$slug)->with('error','Registration for this event is closed.');
        ...
        $events = $categoryIds ? db_connect()->table('events')->select('id,category_id,name,slug,fee,sageian_fee,non_sageian_fee,tax_amount,discount_amount,payment_required,registration_type,min_team_size,max_team_size')->whereIn('category_id',$categoryIds)->where('status','registration_open')->orderBy('name','ASC')->get()->getResultArray() : [];
    ...
    public function store(?string $slug = null)
    {
        ...
        $routeEvent = $slug ? (new EventModel())->bySlug($slug) : null;
        if ($slug && (!$routeEvent || $routeEvent['status'] !== 'registration_open')) return redirect()->to('/events/'.$slug)->with('error','Registration for this event is closed.');
        $categoryId = $routeEvent ? (int)$routeEvent['category_id'] : (int)($input['category_id'] ?? 0);
        $eventId = $routeEvent ? (int)$routeEvent['id'] : (int)($input['event_id'] ?? 0);
