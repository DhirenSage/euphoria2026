...
class ScannerController extends BaseController
{
    ...
    public function index()
    {
        ...
        if(in_array('SUPER_ADMIN',$roles,true)||in_array('EVENT_ADMIN',$roles,true)) { $events=$db->table('events')->whereIn('status',['scheduled','registration_open','live'])->orderBy('event_start','ASC')->get()->getResultArray(); $days=$db->table('event_days')->where('is_active',1)->orderBy('event_date','ASC')->get()->getResultArray(); $gates=$db->table('gates')->where('is_active',1)->get()->getResultArray(); }
    ...
    public function scan()
    {
        ...
        if($eventId<1||$dayId<1||!$gateId||trim((string)($payload['token']??''))==='') return $this->response->setStatusCode(422)->setJSON(['ok'=>false,'status'=>'denied','message'=>'Token, event, event day and gate are required.','csrf'=>csrf_hash()]);
        if(!$db->table('event_days')->where(['id'=>$dayId,'event_id'=>$eventId,'is_active'=>1])->countAllResults()||!$db->table('gates')->where(['id'=>$gateId,'is_active'=>1])->countAllResults())return $this->response->setStatusCode(422)->setJSON(['ok'=>false,'status'=>'denied','message'=>'Select a valid active event day and gate.','csrf'=>csrf_hash()]);
