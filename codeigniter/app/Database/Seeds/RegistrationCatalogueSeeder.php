    ...
    public function run()
    {
        ...
        foreach ($catalogue as $categoryName => $events) {
            ...
            foreach ($events as [$name,$slug,$fee,$type,$minimum,$maximum]) {
                $venue = $categoryName === 'Sports' ? 'University Sports Complex · SAGE University Indore' : ($categoryName === 'Cultural' ? 'Main Auditorium · SAGE University Indore' : 'Seminar Hall · SAGE University Indore');
                $data = ['category_id'=>$categoryId,'name'=>$name,'short_description'=>'Step into '.$name.' and make your EUPHORIA 2K26 moment count.','description'=>$name.' is an official EUPHORIA 2K26 event at SAGE University Indore. It brings students together to compete, perform and learn in a professionally coordinated festival environment.','event_type'=>$categoryName === 'Sports' ? 'sports' : 'competition','registration_type'=>$type,'fee'=>$fee,'sageian_fee'=>$fee,'non_sageian_fee'=>$fee,'payment_required'=>$fee>0?1:0,'capacity'=>250,'min_team_size'=>$minimum,'max_team_size'=>$maximum,'registration_start'=>'2026-01-01 00:00:00','registration_end'=>'2026-09-14 23:59:59','event_start'=>'2026-09-15 10:00:00','event_end'=>'2026-09-17 18:00:00','venue'=>$venue,'eligibility'=>'Open to school and college students with a valid institutional ID.','rules'=>'Carry registration confirmation and photo ID. Report 30 minutes early. Follow coordinator and safety instructions. The organising committee decision is final.','prizes'=>'Winner trophy and merit certificate. Runner-up merit certificate. Participation certificate for eligible participants.','contact_details'=>'EUPHORIA Event Desk · SAGE University Indore','status'=>'registration_open','updated_at'=>$now];
                $existing = $this->db->table('events')->where('slug',$slug)->get()->getRowArray();
                if ($existing) { $this->db->table('events')->where('id',$existing['id'])->update($data); $eventId=(int)$existing['id']; }
                else { $this->db->table('events')->insert($data + ['slug'=>$slug,'is_featured'=>0,'created_at'=>$now]); $eventId=(int)$this->db->insertID(); }
                foreach ([['Day 1','2026-09-15'],['Day 2','2026-09-16'],['Day 3','2026-09-17']] as $day) {
                    $existingDay=$this->db->table('event_days')->where(['event_id'=>$eventId,'event_date'=>$day[1]])->get()->getRowArray();
                    if($existingDay)$this->db->table('event_days')->where('id',$existingDay['id'])->update(['label'=>$day[0],'is_active'=>1,'updated_at'=>$now]);
                    else $this->db->table('event_days')->insert(['event_id'=>$eventId,'label'=>$day[0],'event_date'=>$day[1],'is_active'=>1,'created_at'=>$now,'updated_at'=>$now]);
                }
            }
