...
class DatabaseSeeder extends Seeder
{
    public function run()
    {
        ...
        $eventIds=[];
        foreach ($events as $event) { $db->table('events')->insert(['category_id'=>$cat[$event[0]],'name'=>$event[1],'slug'=>$event[2],'short_description'=>$event[3],'description'=>'A high-energy Euphoria experience designed for students who want to participate, not just watch.','event_type'=>$event[4],'registration_type'=>str_contains($event[2],'cricket')||str_contains($event[2],'football')||str_contains($event[2],'hackathon')?'team':'individual','fee'=>$event[5],'sageian_fee'=>$event[5],'non_sageian_fee'=>$event[5],'payment_required'=>$event[5]>0?1:0,'capacity'=>200,'venue'=>$event[7],'event_start'=>'2026-09-15 10:00:00','event_end'=>'2026-09-17 18:00:00','status'=>$event[6],'is_featured'=>in_array($event[2],['dance-competition','ai-hackathon'],true),'created_at'=>$now,'updated_at'=>$now]); $eventId=$db->insertID(); $eventIds[$event[2]]=$eventId; foreach ([['Day 1','2026-09-15'],['Day 2','2026-09-16'],['Day 3','2026-09-17']] as $day) $db->table('event_days')->insert(['event_id'=>$eventId,'label'=>$day[0],'event_date'=>$day[1],'is_active'=>1,'created_at'=>$now,'updated_at'=>$now]); foreach ([['Full name','participant_name','text',1],['Email address','email','email',1],['Mobile number','mobile','phone',1],['College / institution','college','text',0]] as $n=>$field) $db->table('registration_fields')->insert(['event_id'=>$eventId,'label'=>$field[0],'field_name'=>$field[1],'field_type'=>$field[2],'is_required'=>$field[3],'is_active'=>0,'display_order'=>$n+1,'created_at'=>$now,'updated_at'=>$now]); }
