document.addEventListener('DOMContentLoaded', function () {
  ...
  document.querySelectorAll('[data-permanent-event-delete]').forEach(function(button){button.addEventListener('click',function(){var name=button.dataset.eventName;var typed=window.prompt('Permanent deletion cannot be undone. Type the exact event name to delete all related registrations, payments, QR passes and attendance:\n\n'+name);if(typed!==name){if(typed!==null)window.alert('Event name did not match. Nothing was deleted.');return;}var form=button.closest('[data-permanent-event-form]');form.querySelector('[name="confirm_name"]').value=typed;form.submit();});});
  var selectAll=document.querySelector('[data-event-select-all]');if(selectAll)selectAll.addEventListener('change',function(){document.querySelectorAll('[data-event-checkbox]').forEach(function(checkbox){checkbox.checked=selectAll.checked;});});
  var bulkDelete=document.querySelector('[data-bulk-event-delete]');if(bulkDelete)bulkDelete.addEventListener('click',function(){var selected=document.querySelectorAll('[data-event-checkbox]:checked');if(!selected.length){window.alert('Select at least one event.');return;}var phrase=window.prompt('This will permanently delete '+selected.length+' selected events and all related data. Type DELETE SELECTED to continue.');if(phrase!=='DELETE SELECTED'){if(phrase!==null)window.alert('Confirmation phrase did not match. Nothing was deleted.');return;}var form=document.getElementById('bulk-delete-form');form.querySelector('[name="confirm_phrase"]').value=phrase;form.submit();});
  ...
  if(scannerForm){
    ...
    async function verify(token){if(!token.trim())return;scanButton.disabled=true;scanButton.textContent='Verifying…';
      try {
        ...
        result.className = 'scanner-result ' + (data.ok ? 'result-allowed' : (data.status==='duplicate'?'result-duplicate':(data.status==='upcoming'?'result-upcoming':'result-denied')));
        var detail=data.registration?'<dl class="scan-registration"><div><dt>Participant</dt><dd>'+escapeHtml(data.registration.participant_name)+'</dd></div><div><dt>Registration</dt><dd>'+escapeHtml(data.registration.registration_id)+'</dd></div><div><dt>Event</dt><dd>'+escapeHtml(data.registration.event_name)+'</dd></div><div><dt>Event day</dt><dd>'+escapeHtml(data.registration.event_day_label||'—')+' · '+escapeHtml(data.registration.event_day_date||'—')+'</dd></div><div><dt>Event time</dt><dd>'+escapeHtml(data.registration.event_time||'As scheduled')+'</dd></div><div><dt>Venue</dt><dd>'+escapeHtml(data.registration.venue||'SAGE University Indore')+'</dd></div><div><dt>Institute</dt><dd>'+escapeHtml(data.registration.college)+'</dd></div><div><dt>Mobile</dt><dd>'+escapeHtml(data.registration.mobile)+'</dd></div><div><dt>Email</dt><dd>'+escapeHtml(data.registration.email)+'</dd></div><div><dt>Payment / pass</dt><dd>'+escapeHtml(String(data.registration.payment_status).toUpperCase())+' / '+escapeHtml(String(data.registration.qr_status).toUpperCase())+'</dd></div></dl>':'';
        ...
        result.innerHTML = '<span class="scanner-result-icon">' + (data.ok ? '✓' : (data.status==='upcoming'?'◷':'!')) + '</span><strong>' + (data.ok ? 'ENTRY ALLOWED' : (data.status === 'duplicate' ? 'ENTRY ALREADY RECORDED' : (data.status==='upcoming'?'UPCOMING EVENT — ENTRY NOT OPEN':'ENTRY DENIED'))) + '</strong><p>' + escapeHtml(data.message) + '</p>'+detail+firstEntry;tone(data.ok);if(navigator.vibrate&&data.status!=='upcoming')navigator.vibrate(data.ok?[100]:[180,80,180]);tokenInput.value='';
